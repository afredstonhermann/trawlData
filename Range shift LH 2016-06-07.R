...
## Spp velocity for West Coast US
	source('map2.R') # make maps with a specified polygon border color
	require(maps)
	require(mapdata)
	require(mapproj)
	...
	regs = c('WestCoast_Tri')
	spp = 'Raja binoculata'
		inds = shift$region %in% regs
		inds = shift$region %in% regs & shift$spp %in% spp
	sum(inds) 
	...
	# Move end of Raja binoculata arrow offshore (so doesn't overlap land)
	if(spp == 'Raja binoculata'){
		lon2 = lon2-0.5
	}
...
#	shiftcols = cols[7]
	...
	quartz(width=7, height=7)
	# pdf(width=7, height=7, file=paste('Figures/Sppvelocity_WestCoast_', spp, '_', Sys.Date(), '.pdf', sep=''))
	par(omi = c(0.5,0.5,0,0))
	map2('worldHires', xlim=c(-128,-117), ylim=c(32,49), fill=TRUE, col=bcol, bcol=NA, wrap=FALSE, myborder = 0)
	map2(database='state', add=TRUE, fill=TRUE, col=bcol, bcol=bordercol)
		axis(1); axis(2, las=1); box()
		mtext("Longitude", side=1, line=2.5)
		mtext("Latitude", side=2, line=2.5)

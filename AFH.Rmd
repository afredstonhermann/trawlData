...
if (!file.exists(rdata)){
  ...
  # filtering columns and combining data  
...
``` {r exploring and filtering - lines 176-237 of Malin's code, eval=F}
...
for(i in 1:length(yrs)){ # i = 1:length(yrs)[1] # i = 1

  yr = yrs[i]
  print(yr)
  
  # 1. unique by species for one year (still in loop, vs others outside for loop)
  d_yr = alldata  %>%
    filter(year == yr, wtcpue > 0) %>% # !is.na(alldata$lat), !is.na(alldata$wtcpue): same nrow()
    group_by(regspecies) %>%
    transmute(
      year      = yr,
      variable  = 'lat_minna',
  ...
  # 2.a. unique by species for all years, in tidy form (then rbind)
  d_allyrs = alldata  %>%
    filter(wtcpue > 0) %>% # !is.na(alldata$lat), !is.na(alldata$wtcpue): same nrow()
    group_by(regspecies, year) %>%
    transmute(
      variable  = 'lat_minna',
      value = minna(lat))

  # 2.b. for next variable
  d_allyrs = bind_rows(
    d_allyrs,
    alldata  %>%
      filter(wtcpue > 0) %>% # !is.na(alldata$lat), !is.na(alldata$wtcpue): same nrow()
  ...
  # 3. unique by species for all years, in less tidy form (each var as a new column: most parsimonious! and works if all have same filter criteria)
  d_allyrs = alldata  %>%
    filter(wtcpue > 0) %>% # !is.na(alldata$lat), !is.na(alldata$wtcpue): same nrow()
    group_by(regspecies, year) %>%
    transmute(
...
# Min lat
	btemp = aggregate(alldata$lat[inds & alldata$wtcpue>0], by = list(regspecies = alldata$regspecies[inds & alldata$wtcpue>0]), FUN = minna)
	  names(btemp)[2] = yrs[i]
	minlat = merge(minlat, btemp, all.x=TRUE)
